@extends('layouts.app')
@section('title', 'Sửa Client')
@section('page-title', 'Sửa Client: ' . $client->name)
@section('breadcrumb')
    ...
    <div class="col-lg-8">
        <div class="card card-outline card-warning shadow-sm">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-edit mr-2"></i>Chỉnh sửa thông tin Client</h3>
            </div>
            <form method="POST" action="{{ route('clients.update', $client) }}" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group text-center">
                        <img src="{{ Storage::url($client->logo_path) }}" alt="Logo"
                             class="img-thumbnail mb-2" style="max-height:80px;">
                        <br><small class="text-muted">Logo hiện tại</small>
                    </div>
                    @endif
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="report_frequency"
                                        class="form-control @error('report_frequency') is-invalid @enderror" required>
                                    @foreach(['weekly'=>'Hàng tuần','biweekly'=>'2 tuần/lần','monthly'=>'Hàng tháng','quarterly'=>'Hàng quý'] as $val => $label)
                                        <option value="{{ $val }}" {{ old('report_frequency', $client->report_frequency) === $val ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="form-group">
                        <label>Logo mới <small class="text-muted">(để trống = giữ logo cũ)</small></label>
